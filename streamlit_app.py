...
# Inicializar cliente MQTT\client = mqtt.Client()
client.on_connect = on_connect
client.on_message = on_message
client.connect(BROKER, PORT, 60)
client.loop_start()

# Título de la app\st.title("Sistema Domótico para Control de Plantas")

# Contenedores\last_values_container = st.empty()
...
def update_dashboard():
    ...
    with last_values_container.container():
        ...
        st.text(f"Color de hojas: {last_values['leaf_color']}")
        st.text(f"Estado del motor de agua: {last_values['water_motor']}")
        st.text(f"Estado del sistema: {last_values['alert']}")
    ...
    # Graficar datos\    with graph_container.container():
